...
For the different type of deep learning algorithm a simple misclassification error rate will be use as the accuracy.


\subsubsection{Time elapsed}
A elapsed time was also provided to give an idea of the training time. Ensure not to generalised this characteristic since it depends on the data, the computer, the seed, etc.
...
\paragraph{Windows} \textit{The only supported installation method on Windows is "conda". It means you should install install Anaconda version 3.x before doing these steps.}

\textbf{First step}, is the installation and download of the Keras files from GitHub :
\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
...
\begin{alltt}
\hlkwd{library}\hlstd{(keras)}
\hlkwd{install_keras}\hlstd{(}\hlkwc{method} \hlstd{=} \hlkwd{c}\hlstd{(}\hlstr{"auto"}\hlstd{,} \hlstr{"virtualenv"}\hlstd{,} \hlstr{"conda"}\hlstd{),}
  \hlkwc{conda} \hlstd{=} \hlstr{"auto"}\hlstd{,}
  \hlkwc{version} \hlstd{=} \hlstr{"default"}\hlstd{,}
  \hlkwc{tensorflow} \hlstd{=} \hlstr{"default"}\hlstd{,}
  \hlkwc{extra_packages} \hlstd{=} \hlkwd{c}\hlstd{(}\hlstr{"tensorflow-hub"}\hlstd{))}
\end{alltt}
\end{kframe}
\end{knitrout}
...
The dataset iris dont exist direc
\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
...
\hlkwd{c}\hlstd{(test_data, test_labels)} \hlopt{%<-%} \hlstd{imdb}\hlopt{$}\hlstd{test}
...
\hlkwd{decode_review}\hlstd{(train_data[[}\hlnum{5}\hlstd{]])}
...
\hlstd{train_data_pad} \hlkwb{<-} \hlkwd{pad_sequences}\hlstd{(}
  \hlstd{train_data,}
...
\hlstd{test_data_pad} \hlkwb{<-} \hlkwd{pad_sequences}\hlstd{(}
  \hlstd{test_data,}
  \hlkwc{value} \hlstd{= word_index_df} \hlopt{%>%} \hlkwd{filter}\hlstd{(word} \hlopt{==} \hlstr{"<PAD>"}\hlstd{)} \hlopt{%>%} \hlkwd{select}\hlstd{(idx)} \hlopt{%>%} \hlkwd{pull}\hlstd{(),}
  \hlkwc{padding} \hlstd{=} \hlstr{"post"}\hlstd{,}
  \hlkwc{maxlen} \hlstd{= maxlen}
...
\hlstd{model} \hlkwb{<-} \hlkwd{keras_model_sequential}\hlstd{()}
\hlstd{model} \hlopt{%>%}
  \hlkwd{layer_embedding}\hlstd{(}\hlkwc{input_dim} \hlstd{= max_features,} \hlkwc{output_dim} \hlstd{=} \hlnum{256}\hlstd{)} \hlopt{%>%}
  \hlkwd{layer_lstm}\hlstd{(}\hlkwc{units} \hlstd{=} \hlnum{128}\hlstd{,}
             \hlkwc{input_shape} \hlstd{=} \hlnum{256}\hlstd{,}
             \hlkwc{batch_size} \hlstd{=} \hlnum{256}\hlstd{)}\hlopt{%>%}
  \hlkwd{layer_dropout}\hlstd{(}\hlkwc{rate} \hlstd{=} \hlnum{0.2}\hlstd{)} \hlopt{%>%}
