...
neuralnet(formula, Y~X1+...+Xn, hidden = 1, threshold = 0.01, stepmax = 1e+05, rep = 1,
...
\textit{Important default parameters of a FNN model with the neuralnet package:}
<<eval= FALSE, message = FALSE>>=
nnet(X, Y, size, linout=FALSE, entropy=FALSE, softmax=FALSE, maxit = 100, decay = 0)
@
...
<<eval= FALSE, message = FALSE>>=
dbn.dnn.train(X, Y, hidden=c(5), activationfun="sigm", learningrate=0.5,
              momentum=0.5, learningrate_scale=1, output="softmax",
...
load("C:/Users/adrie/OneDrive/Desktop/Git/Deep_learning_r/DeepLearninginR/FNN_save.RData")
@
\label{sec:examples}
